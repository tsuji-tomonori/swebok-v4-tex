...
この実行では `fig:ch03-12` だけを個別に生成する。HTML/SVG/Canvas/TikZ/スクリーンショット等の決定的レンダリングで代替しない。

重要:
- built-in `image_gen` を Codex から呼び出して、この1枚を新規生成する。
- 生成後、最新の `$CODEX_HOME/generated_images/...` の PNG をプロジェクト側へコピーする。
- コピー先: `/home/t-tsuji/project/swebok-v4-tex/tex/assets/generated_figures/ch03/fig-ch03-12.png`
- このプロンプト: `/home/t-tsuji/project/swebok-v4-tex/tex/imagegen_prompts/ch03/fig-ch03-12.md`
...
Meaning to convey:
温かいオフホワイト背景、控えめなアクセントカラー、A4本文幅の比較表風図を作成する。列は「レビュー」「静的分析」「形式的分析」「シミュレーション」「プロトタイプ」。行は「主な目的」「必要な成果物」「強み」「注意点」。各セルは短い日本語で埋める。講義ノート風、細線、印刷で読みやすい。

完了条件:
- `/home/t-tsuji/project/swebok-v4-tex/tex/assets/generated_figures/ch03/fig-ch03-12.png` が存在する。
- `/home/t-tsuji/project/swebok-v4-tex/tex/swebok_v4_ch01_06_ja_imagegen_manifest.tsv` に `fig:ch03-12` の success または failed 行を追記する。
