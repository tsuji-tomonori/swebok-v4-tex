...
この実行では `fig:ch02-04` だけを個別に生成する。HTML/SVG/Canvas/TikZ/スクリーンショット等の決定的レンダリングで代替しない。

重要:
- built-in `image_gen` を Codex から呼び出して、この1枚を新規生成する。
- 生成後、最新の `$CODEX_HOME/generated_images/...` の PNG をプロジェクト側へコピーする。
- コピー先: `/home/t-tsuji/project/swebok-v4-tex/tex/assets/generated_figures/ch02/fig-ch02-04.png`
- このプロンプト: `/home/t-tsuji/project/swebok-v4-tex/tex/imagegen_prompts/ch02/fig-ch02-04.md`
...
生成対象:
Title: ビューとビューポイントの関係図
Layout: 中央に主概念を置き、周辺に関連項目を配置する放射状の図解。
Central concept: アーキテクチャ記述
...
Meaning to convey:
温かいオフホワイト背景、控えめなアクセントカラー、A4 本文幅の概念図を作成する。中央に「アーキテクチャ記述」を置き、そこから「論理ビュー」「開発ビュー」「プロセスビュー」「配置ビュー」「シナリオビュー」へ枝分かれさせる。各ビューの上に、そのビューを定める「ビューポイント」を小さなラベルで示す。右側に「利害関係者の関心事に答える」と注記する。日本語ラベル、講義ノート風。

完了条件:
- `/home/t-tsuji/project/swebok-v4-tex/tex/assets/generated_figures/ch02/fig-ch02-04.png` が存在する。
- `/home/t-tsuji/project/swebok-v4-tex/tex/swebok_v4_ch01_06_ja_imagegen_manifest.tsv` に `fig:ch02-04` の success または failed 行を追記する。
