...
この実行では `fig:ch05-13` だけを個別に生成する。HTML/SVG/Canvas/TikZ/スクリーンショット等の決定的レンダリングで代替しない。

重要:
- built-in `image_gen` を Codex から呼び出して、この1枚を新規生成する。
- 生成後、最新の `$CODEX_HOME/generated_images/...` の PNG をプロジェクト側へコピーする。
- コピー先: `/home/t-tsuji/project/swebok-v4-tex/tex/assets/generated_figures/ch05/fig-ch05-13.png`
- このプロンプト: `/home/t-tsuji/project/swebok-v4-tex/tex/imagegen_prompts/ch05/fig-ch05-13.md`
...
生成対象:
Title: 従来型とシフトレフトの比較図
Layout: 左から右へ進む工程図または循環を示すフロー図。
Central concept: 要求、設計、実装、後半のテスト

Visible Japanese labels. Use these labels exactly where possible:
- 要求、設計、実装、後半のテスト
- 要求からテスト設計、単体、結合、回帰、自動化、監視
- 欠陥を早く見つけるほど修正コストが下がる

Meaning to convey:
温かいオフホワイト背景、控えめなアクセントカラー、A4 本文幅の比較図を作成する。上段に従来型の流れ「要求、設計、実装、後半のテスト」を横矢印で示す。下段にシフトレフトの流れ「要求からテスト設計、単体、結合、回帰、自動化、監視」が早期から並走する様子を示す。右側に「欠陥を早く見つけるほど修正コストが下がる」と注記する。日本語ラベル、講義ノート風。

完了条件:
- `/home/t-tsuji/project/swebok-v4-tex/tex/assets/generated_figures/ch05/fig-ch05-13.png` が存在する。
- `/home/t-tsuji/project/swebok-v4-tex/tex/swebok_v4_ch01_06_ja_imagegen_manifest.tsv` に `fig:ch05-13` の success または failed 行を追記する。
