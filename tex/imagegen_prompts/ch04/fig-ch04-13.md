...
この実行では `fig:ch04-13` だけを個別に生成する。HTML/SVG/Canvas/TikZ/スクリーンショット等の決定的レンダリングで代替しない。

重要:
- built-in `image_gen` を Codex から呼び出して、この1枚を新規生成する。
- 生成後、最新の `$CODEX_HOME/generated_images/...` の PNG をプロジェクト側へコピーする。
- コピー先: `/home/t-tsuji/project/swebok-v4-tex/tex/assets/generated_figures/ch04/fig-ch04-13.png`
- このプロンプト: `/home/t-tsuji/project/swebok-v4-tex/tex/imagegen_prompts/ch04/fig-ch04-13.md`
...
生成対象:
Title: 性能分析とチューニングの流れ
Layout: 左から右へ進む工程図または循環を示すフロー図。
Central concept: 性能要求の確認
...
Meaning to convey:
温かいオフホワイト背景、控えめなアクセントカラー、A4 本文幅の循環図を作成する。「性能要求の確認」「計測」「ボトルネック特定」「改善案」「実装変更」「再計測」を円環で示す。中央に「推測ではなく測定」と書く。横に「実行時間」「メモリ」「I/O」「ネットワーク」の測定対象を小さく配置する。日本語、講義ノート風。

完了条件:
- `/home/t-tsuji/project/swebok-v4-tex/tex/assets/generated_figures/ch04/fig-ch04-13.png` が存在する。
- `/home/t-tsuji/project/swebok-v4-tex/tex/swebok_v4_ch01_06_ja_imagegen_manifest.tsv` に `fig:ch04-13` の success または failed 行を追記する。
