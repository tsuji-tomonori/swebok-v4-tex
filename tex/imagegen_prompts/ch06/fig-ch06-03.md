...
この実行では `fig:ch06-03` だけを個別に生成する。HTML/SVG/Canvas/TikZ/スクリーンショット等の決定的レンダリングで代替しない。

重要:
- built-in `image_gen` を Codex から呼び出して、この1枚を新規生成する。
- 生成後、最新の `$CODEX_HOME/generated_images/...` の PNG をプロジェクト側へコピーする。
- コピー先: `/home/t-tsuji/project/swebok-v4-tex/tex/assets/generated_figures/ch06/fig-ch06-03.png`
- このプロンプト: `/home/t-tsuji/project/swebok-v4-tex/tex/imagegen_prompts/ch06/fig-ch06-03.md`
...
生成対象:
Title: 運用計画とCONOPSの関係
Layout: 中央に主概念を置き、周辺に関連項目を配置する放射状の図解。
Central concept: 開発開始の意思決定
...
Meaning to convey:
左に「開発開始の意思決定」、中央に「概念文書」「運用保守計画」「運用構想 CONOPS」、右に「変更要求」「問題報告」「リリース計画」「運用支援」を置き、運用計画が開発初期から始まることを矢印で示す。

完了条件:
- `/home/t-tsuji/project/swebok-v4-tex/tex/assets/generated_figures/ch06/fig-ch06-03.png` が存在する。
- `/home/t-tsuji/project/swebok-v4-tex/tex/swebok_v4_ch01_06_ja_imagegen_manifest.tsv` に `fig:ch06-03` の success または failed 行を追記する。
