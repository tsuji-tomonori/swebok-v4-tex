...
def mirror_chapter01_content() -> None:
    ...
    write_content(root, opening, opening.read_text(encoding="utf-8"))
    ...
    review_questions = source_dir / "14_章末確認問題.tex"
    root.joinpath("review_questions.tex").write_text(
        generated_tex(review_questions, review_questions.read_text(encoding="utf-8")),
        encoding="utf-8",
    )

    inputs = [root / "content.tex"]
    ...
    inputs.append(root / "review_questions.tex")

    root.joinpath("chapter.tex").write_text(
        "\n".join(
            [
